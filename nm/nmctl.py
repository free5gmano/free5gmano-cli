    ...
    if response.status_code == 201:
        ...
        if download:
            example_type = click.prompt('Choice example what you want to download. Default is free5gc-stage-1 ', default='free5gc-stage-1', 
                type=click.Choice(['free5gc-stage-1', 'free5gc-stage-2', 'free5gc-stage-3.0.4'], case_sensitive=False), show_choices=True)
            if example_type:
...
def create_subscriptions(type, nss_instance_id):
    if type == 'fm':
        response = api.create_fm_subscriptions(nss_instance_id)
        if response.status_code == 201:
            click.echo("notification Id: "+response.json()['data']['notificationId'])
            consumers = api.create_consumers()
            click.echo("Listen Kafka:")
            click.echo(settings.Kafka_URL + "/subscription")
            data = {"topics": ["fault_alarm"]}
            subscribe = api.create_topic(url=settings.Kafka_URL + "/subscription", data=data)

            while 1:
                record = api.get_record(url=settings.Kafka_URL + "/records")
                if record.json():
        ...
        subscribe = api.create_topic(url=consumers.json()['base_uri'] + "/subscription", data=data)

        while 1:
            record = api.get_record(url=consumers.json()['base_uri'] + "/records")
            if record.json():
                click.echo(base64.b64decode(record.json()[-1]['value']).decode())
                break
                time.sleep(3)
...
def get_subscriptions():
    ...
    if response.status_code == 200:
        for i in response.json():
            ...
            output = pd.DataFrame(data=data)
        click.echo(output.to_string(index=False, columns=['notificationId', 'filter']))
